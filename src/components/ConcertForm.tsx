...
function ConcertForm({
  ...
  // Choisir un fichier ouvre la modale de recadrage (pas d'upload ici)
  ...
  // Le recadrage validé nous donne l'image coupée (blob) → on l'uploade
  ...
  function addLineupRow() {
    setLineup((previous) => [
      ...previous,
      { mode: "suivi", groupeId: "", groupeName: "" },
    ]);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (lineupRows.length > 0) {
      const { error: lineupError } = await supabase
        ...
        .insert(lineupRows);
      if (lineupError) {
        setErrorMessage(lineupError.message);
        setIsSaving(false);
        return;
      }
    }
        ...
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          <div className="modal-head">
            <div className="modal-title">
              {concert ? "Modifier" : "Nouveau concert"}
            </div>
          ...
          <form onSubmit={handleSubmit} className="form-grid">
            ...
            <div className="field">
                ...
                <div key={index} className="lineup-row">
                  <select
                    ...
                    onChange={(e) =>
                      updateLineupRow(index, {
                        mode: e.target.value as "suivi" | "libre",
                      })
                    }>
                  ...
                  {row.mode === "suivi" ? (
                    <select
                      value={row.groupeId}
                      onChange={(e) =>
                        updateLineupRow(index, { groupeId: e.target.value })
                      }>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateLineupRow(index, { groupeName: e.target.value })
                      }
                    />
                  )}
              ...
              <button
                type="button"
                className="btn-ghost lineup-add"
