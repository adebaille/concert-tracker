...
import { syncPastConcerts } from "../lib/concerts";
...
function HomePage() {
  ...
  const [lineup, setLineup] = useState<
    { concert_id: number; groupe_id: number | null }[]
  >([]);
  const [groupePhotos, setGroupePhotos] = useState<
  ...
  useEffect(() => {
    async function loadHome() {
      ...
      const concertsData = await syncPastConcerts(
        (concertRows ?? []) as ConcertRow[],
      );
  ...
  const nextPosterPhoto = (() => {
    ...
    const followed = lineup.filter(
      (l) => l.concert_id === nextConcert.id && l.groupe_id !== null,
    );
      ...
      <section className="hero">
        <div>
          <h1 className="hero-title">
            <span className="gradient">CONCERT</span>
            <br />
        ...
        <div className="hero-right">
          <span>
            {concerts.length} concerts · {groupesCount} groupes
          </span>
        </div>
      </section>
            ...
            <div className="nc-info">
              <div
                ...
                <div>
                  ...
                  <div className="nc-value big">
                    {formatConcertDate(nextConcert.event_date)}
                  </div>
                  <div
                    className="nc-value"
                    style={{
                ...
                <div>
                  ...
                  <div
                    ...
                    <span style={{ fontSize: 13 }}>
                      {participantsLabel(nextParticipants)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div key={entry.key} className="act">
                ...
                <div>
                  ...
                  <div className="act-time">
                    {formatConcertDate(entry.createdAt)}
                  </div>
                </div>
              </div>
            ))}
